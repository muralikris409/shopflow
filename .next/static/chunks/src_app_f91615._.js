(globalThis.TURBOPACK = globalThis.TURBOPACK || []).push(["static/chunks/src_app_f91615._.js", {
    ...
    const handlePayment = async ()=>{
        ...
        try {
            ...
            const options = {
                ...
                handler: async function(response) {
                    const { razorpay_payment_id, razorpay_signature, razorpay_order_id } = response;
                    const verificationResult = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$app$2f$service$2f$PaymentService$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["verifyPayment"])(orders?.orders?.order?.id || orders?.orders.id, razorpay_order_id, razorpay_payment_id, razorpay_signature);
                    if (verificationResult.success) {
                        const res = await handleVerify(orders?.orders?.order?.id || orders?.orders.id, razorpay_order_id, razorpay_payment_id, razorpay_signature);
                        router.push(`/orders/success?dtrcpt=${encodeURIComponent(JSON.stringify(res?.order))}`);
                    } else {
...
"[project]/src/app/checkout/page.js [app-client] (ecmascript)": (function(__turbopack_context__) {
...
//# sourceMappingURL=src_app_f91615._.js.map